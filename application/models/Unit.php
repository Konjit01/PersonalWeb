...
class Unit extends CI_Model{
    ...
    public function get_units_by_subcategory( $limit = FALSE, $offset=FALSE, $id){
        $sql = "SELECT * FROM `units` JOIN `users` ON `units`.`user_id` = `users`.`id` AND `units`.`subcategory_id` = $id ;";
        $query = $this->db->query($sql);
