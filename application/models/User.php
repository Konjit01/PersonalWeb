...
class User extends CI_Model {
    ...
    public function password_recovery() {
        ...
        if ($result->num_rows() === 1) {
            $to = $email;
            $id = $result->row(0)->id;
            $code = $result->row(0)->code; //print('157 user_id in model'  .$code);
            $subject = "Recover Your Account";
            $message = '<p>You requested for password recovery @ TechWisdom</p>';
            $message .= "<p>Your Account:</p>
					<p>Email: " . $email . "</p>
				    <p>Please click the link below to recover your account.</p>";
            $message .= "<h4><a href='" . base_url() . "users/update_password/" . $id . "/" . $code . "'>Recover My Account</a></h4>";
            ...
            if ($is_successful === TRUE) {
                //echo "Has it reached line 171 "; die();
                return true;
            } else {
                   //echo "Email was not sent "; die();
                return false;
            }
        }
    }
